...
2. Будь внимателен, запуск инструмента выполняется при помощи тега <tool_call>, а не <tools>. 
3. Запрещено добавлять поле price в schema для исследования. Цена будет извлекаться отдельно, не нужно ее указывать как параметр. 
4. Запросы в поле query пиши максимально кратко, как в поиск по ключевым словам. Не добавляй контекст ("для ПК", "в машину", "игровой"). 
...
3. В schema указывай только фактические поля. Например для электроники - частота, объем, тип. Запрещены оценочные поля "качество", "выгодность" и подобное.
