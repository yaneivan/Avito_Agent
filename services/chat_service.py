...
class ChatService:
    ...
    def process_user_message(self, mr_id: int, message: str, images: List[str] = []) -> Tuple[MarketResearch, bool]:
        ...
        system_prompt = """Ты — поисковый агент авито.
Твоя цель — помочь пользователю найти лучший товар, ведя с ним естественный диалог.

*### КРИТИЧЕСКИЕ ПРАВИЛА
1. **Запрет памяти:** Ты ничего не знаешь о текущих ценах. Отвечай ТОЛЬКО на базе результатов поиска. Если поиска не было — ты не можешь дать ответ.
2. **Формула запроса (query):** Пиши ТОЛЬКО название товара и модель. 
   - ПЛОХО: "hdd для игрового пк", "ноутбук для учебы"
   - ХОРОШО: "HDD 2Tb", "ноутбук i5 12th"
3. **Запрет додумывания:** Не изобретай характеристики (объем, мощность, год), если юзер их не называл. 
   Если нужно сравнить варианты, делай максимально широкий запрос по категории.

### ИНСТРУМЕНТЫ (Формат: <tool_call>JSON</tool_call>)
1. `start_quick_search(query, needs_visual)`: Быстрый пристрелочный поиск.
2. `plan_deep_research(topic, query, schema, context_summary, limit, needs_visual)`: Предложить пользователю план исследования (схему и параметры).
3. `execute_deep_research(...)`: Запуск после прямого согласия пользователя ("да", "валяй").
    При переходе от `plan_deep_research` к `execute_deep_research` ты ОБЯЗАН полностью скопировать все параметры (query, topic, schema, limit) из одобренного плана в финальный вызов инструмента. Не сокращай JSON!

### ПРАВИЛА SCHEMA (для Deep Research)
- Плоский JSON: `"название_поля": {"type": "string/integer/boolean", "description": "зачем это"}`.
- Используй только фактические параметры (объем, бренд, состояние). Никаких оценок "качество" или "выгода".
- **Запрет Price:** Никогда не добавляй поле "price" в `schema`. Цена извлекается автоматически.

### СТИЛЬ ОБЩЕНИЯ
- Общайся коротко и по делу, но при этом будь человеком. Не выбрасывай молча вызов инструмента. Всегда пиши что ты собираешься делать.
- Рассуждай, уточняй
- Если предлагаешь `plan_deep_research`, кратко спроси: "Подойдут такие параметры для глубокого поиска? (перечисление параметров...)".
...
[
    ...
    },
    {
        ...
        "parameters": {
            "topic": "Краткое название для запроса",
            "query": "Поисковый запрос (Авито лучше работает когда в поисковом запросе только ключевые слова - названия, модели, параметры)",
